...
$( document ).ready(function () {
	...
	//var g_windowHeight = Number($('#game1').height()) + 150 ;
	var g_boxsize;
	var g_arraybox = [] ;
	var g_arrayrecord = [] ; //記錄目前拼圖的狀態
	var g_temptime = 0 ;
	var g_temptapnum = 0 ;
	var g_imgurl = 'images/a01.jpg';
	var g_size;
	var g_timer;
	var g_timer2;
	...
	function _puzzle() {
		...
		if (g_windowWidth < g_windowHeight) {
			if (g_windowWidth >= 420) {
				g_size =  Math.round((420 - 50) / check_rowcol);
			} else {
				g_size =  Math.round((g_windowWidth - 50) / check_rowcol);
			}
			_boximg(g_size,g_imgurl); //設定box的位置及大小
		} else {
			if (g_windowHeight >= 420) {
				g_size =  Math.round((420 - 50) / check_rowcol);
			} else {
				g_size =  Math.round((g_windowHeight - 50) / check_rowcol);
			}
			_boximg(g_size,g_imgurl); //設定box的位置及大小
		} 	
		...
		console.log('g_windowHeight',g_windowHeight,'g_windowWidth:',g_windowWidth,'margin-left:',((g_windowWidth - 30-(g_size * g_divcolnum))/2),'g_size:',g_size);  //test
		alert('g_windowHeight' + g_windowHeight +'g_windowWidth:' + g_windowWidth + 'margin-left:' +((g_windowWidth - 30-(g_size * g_divcolnum))/2)+'g_size:'+g_size);  //test
	...
	function _marker() {
	    for (i = 1; i < g_divallnum; i++) {
			...
			//字型大小
			marker_fontsize = 30 - g_divallnum ;
			if ( marker_fontsize < 14 ) {
			    marker_fontsize = 14 ;
			}
			
			$(tt).css({'font-size': marker_fontsize +'px','color':'rgb(255,255,255)',
